...
export default function FellowChatPage() {
  ...
  const fetchFellows = async () => {
    try {
      ...
      if (data.fellows) {
        ...
        setFellows(
          data.fellows.filter((f: Fellow) => f._id !== session?.user?.id)
        );
      }
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Chat with other fellows in your cohorts
          </p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Select a fellow to start a one-on-one conversation
              </DialogDescription>
            </DialogHeader>
                ...
                {fellows.map((fellow) => (
                  ...
                  >
                    <Avatar className="h-10 w-10">
                      ...
                      <AvatarFallback>{fellow.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
